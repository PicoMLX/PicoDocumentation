...
## Summary
Use this page as the cheat sheet for the current HTTP surface in Pico AI Server. It lists the routes that are actually registered in the source today, plus the limits that matter before you wire a client to them.
...
| `/v1/responses/:response_id*` | Helper routes exist in source, but are not ready for client documentation |
| `/v1/conversations/*` | Routes exist in source, but most handlers are placeholders |
...
| `408` | Request timeout | Used when generation is cancelled |
...
- `Serve Pico web chat` is present in settings, but the root static middleware is still installed unconditionally in the current source. Treat that toggle as under validation.
