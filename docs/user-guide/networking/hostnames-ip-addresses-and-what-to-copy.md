...
- Choose the safer server address for your client.

## Before you start
- Pico AI Server must be running.
- If you are sharing to another device, LAN Mode must be on.
...
3. Prefer a host name when your clients resolve local names reliably.
4. Prefer the IP-based server address when you need the least surprising copy-and-paste result.
   The menu extra copy button uses an IP-based address.
5. When you share the address, include the scheme and port, for example `http://192.168.x.y:11434`.
...
If it returns JSON, the chosen server address is good enough for client setup.
...
Compare both values, then decide which one you want to share.
