...
Bonjour discovery lets Pico clients find Pico AI Server without typing a server address first. In the current source, Pico AI Server advertises the service type `_pico._tcp.` and includes the IP address, port, local host name, and a stable server identifier in the TXT record.

## What you'll do
- Check whether Bonjour discovery is enabled.
- Browse for Pico AI Server on the LAN.
- Keep a manual fallback ready.

## Before you start
- `Enable Bonjour broadcasting` only works when `Allow local network connections` is on.
- Discovery is optional. Clients still need a manual server-address path.
...
7. Build your client flow around these fields:
   - `IPAddress`
   - `Port`
   - `LocalHostName`
   - `ServerIdentifier`
