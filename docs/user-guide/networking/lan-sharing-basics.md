...
The default port is `11434`. A client on another device needs a routed path to the host Mac — same Wi-Fi or wired LAN is the simple case.
...
5. From another device on the same network, test:
   ...
   Replace `SERVER` with the address from step 4.

6. If that works, your LAN path is good enough for client setup.

## Verify it worked

The second device gets a JSON response from `GET /v1/models`. To see the full experience, load `http://SERVER:11434/` in a browser on that device — the WebUI should appear.
